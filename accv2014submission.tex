...
\begin{figure}[t!]
\begin{center}
   \includegraphics[width=\linewidth]{figures/bias3.eps}
\end{center}
   \vspace{-0.6cm}
   ...
   shows-up as a seam-line across which colors slightly change. [d] Scatter-plot of
   ...
   [b] Splitting the field of view in half, to {\em corrected} pixels from either
...
\begin{equation}
 ...
 \label{eq:Bk}
\end{equation}
The back-projected score of voxel $k$ is null, if $k$ is not observed by at least two viewpoints. This score is null also if $s(r)=0$ for any $r\in\rho_k$. If all $r\in\rho_k$ have same score $s$, then $B_k=s$. Equation~(\ref{eq:Bk}) carves-out voxles that contradict support for clouds.
...
We used SIFT descriptors~\cite{SIFT} and the radiance in the each color channel. Element $q$ of ${\bf v}(r,t)$ is $v_q(r,t)$. The values of this element, for all rays that intersect voxel $k$, is \mbox{${\cal V}_q(k,t)\equiv\{v_q(r,t)\}_{r\in\rho_k}$}.
