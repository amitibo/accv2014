...
of each camera $c$ is known by GPS, and the internal parameters $\Psi_c$ are pre-calibrated in the lab. However, the orientation (yaw, pitch and roll angle vector ${\bf \Theta}_c$) is loosely set, as the camera is placed in the field. We calibrate the orientation by automatically detecting and tracking extra-terrestrial (XT) objects (Sun and/or Moon)~\cite{Seiz2002,lalonde}, across day or night in the hemispherical field of view. Using astronomical charts per time $t$ and ${\bf l}_c$, an XT object is known to be at angle vector (zenith, azimuth) ${\bm \theta}^{\rm XT}(t)$ relative to a global coordinate system. Given camera orientation ${\bf \Theta}_c$, a ray from ${\bm \theta}^{\rm XT}(t)$ should project to pixel:
\begin{equation}
...
\;,
 \label{eq:hatThetac}
\end{equation}
which we optimize using an exhaustive or gradient descent with null initialization.
...
In our experiment, the camera units were installed on various rooftops and ran for weeks, capturing 70GB of sky imagery.
\begin{figure}[t!]
\begin{center}
...
We had found that simple color consistency was not enough when tackling difficult cloud structures such as high-altitude ciruss due to spatial ambiguity and very low baseline between the cameras. In such cases spatial information would be a natural way to incorporate a multi-view stereo approach into our space-carving scheme.
...
128-bin dense SIFT descriptors were chosen to maintain robustness under scale and view variations.
Per voxel $k$, the set of measured radiances is \mbox{${\cal I}_t(k)\equiv\{\hat I_t(r)\}_{r\in\rho_k}$}. Across viewpoints, the measured variance of this set is ${\rm VAR}[{\cal I}_t(k)]$, per $t$. Define a photo-consistency score
Similarity score is computed in a simmilar manner as in Eq.~(\ref{eq:Ak}):
\begin{equation}
 P_k= \exp\left(
 ...
 \label{eq:Dist}
\end{equation}
where $\sigma'^2$ is a scale parameter, and $bin=\{1..128\}$. ${\rm VAR}[{\cal SIFT}_t^{\rm bin}(k)]$ computes the variance at bin. Overall, the total cloud-score of a voxel is $T_k=B_kP_k$.
...
We first applied computer simulations to test the camera network
concept. The simulations allow us to quantify the reconstruction success. They
also enable us to check the limits imposed by the static sun blocker.
In the following we describe the simulation setup: camera images were
synthesized using the SHDOM~\cite{Evans1998} radiative transfer
solver. For the atmosphere, we used the cumulus clouds example distributed
with the SHDOM code which was produced using Large Eddy Simulations (LES).
The atmosphere spans $8 \times 8 {\rm km}$ and the clouds are distributed
between the heights of $500 {\rm m}$ to $1500 {\rm m}$.
We synthesized 100 cameras equally spaced on a $2.5 \times 2.5{\rm km}^2$
grid. We performed multiple simulations each time using only a random subset
of the camera network. The simulations were performed with and without
a sun blocker. The reconstruction score was calculated using the following
equation:
\begin{equation*}
  {\rm Score}=\frac{A \cap B}{A \cup B},
\end{equation*}
where $A$ is the estimated cloud formation and $B$ is the ground truth
cloud formation given in the LES example. Note that the LES example
...
Fig.~\ref{fig:simulations} summarizes the results
of the simulations. As expected of space carving,
results improve fast from 2 to 10 cams. Even when sun blocker is applied,
...
\begin{figure}
  \begin{center}
    \includegraphics{figures/simulations.pdf}
    \caption{Reconstruction success as function of camera number. The
      solid blue line marks the simulations without sun blocker. The
      dashed red line markd the simulations with sun blocker applied.
      The transparent fill marks the standard deviation of repeated
      simulations.
    }
